...
function translationInstructions(glossary) {
  ...
  return [
    ...
    "Translate every Italian utterance into natural English, preserving meaning, tone, names, and numbers.",
    "Respond only with the English translation. Do not answer the speaker or add commentary.",
    "Keep pace with the speaker and use concise phrasing suitable for live captions.",
    terms,
  ].join(" ");
}

function sessionUpdate(model, glossary) {
  return {
    type: "session.update",
    session: {
      type: "realtime",
      model,
      instructions: translationInstructions(glossary),
...
export class OpenAIRealtimeSession {
  constructor({ apiKey, model = "gpt-realtime", glossary = [], onEvent, onStatus, logger = console }) {
    if (!apiKey) {
      throw new AppError(
        503,
  ...
  connect() {
    ...
    socket.on("open", () => {
      ...
      this.reconnectAttempt = 0;
      socket.send(JSON.stringify(sessionUpdate(this.model, this.glossary)));
  ...
  updateGlossary(glossary) {
    ...
    if (this.socket?.readyState === OPEN) {
      this.socket.send(JSON.stringify(sessionUpdate(this.model, this.glossary)));
    }
  }
...
export function createOpenAIRealtimeFactory(config = {}) {
  return ({ glossary, onEvent, onStatus }) =>
