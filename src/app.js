...
function broadcast(room, payload, { audienceOnly = false } = {}) {
...
export function createLinguaServer(options = {}) {
  ...
  app.use((request, response, next) => {
    response.set({
      ...
      "Content-Security-Policy":
        "default-src 'self'; img-src 'self' data:; style-src 'self'; script-src 'self'; connect-src 'self' ws: wss:; worker-src 'self' blob:",
    });
    next();
  });
  ...
  app.post("/api/rooms", (request, response, next) => {
    try {
      const { room, hostToken } = rooms.create({ glossary: request.body?.glossary });
  ...
  app.patch("/api/rooms/:code", (request, response, next) => {
    try {
      const room = rooms.get(request.params.code);
    ...
    const operation = (async () => {
      ...
      try {
        realtime = realtimeFactory({
          ...
          onEvent: (event) => {
            if (event.type === "transcript.done") rooms.addHistory(room, event);
  ...
  return { app, server, rooms, startRoom, endRoom, close };
}
