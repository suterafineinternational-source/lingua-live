      ...
      if (!client.backpressureNotified) {
        ...
        sendJson(client.ws, {
          ...
          error: {
            ...
            message: "Audio was skipped because this listener could not keep up.",
            retriable: true,
          },
        });
      }
...
export function createLinguaServer(options = {}) {
  ...
  app.get("/metrics", (_request, response) => {
    ...
    const lines = [
      "# HELP lingua_active_rooms Current non-ended rooms",
      "# TYPE lingua_active_rooms gauge",
      `lingua_active_rooms ${activeRooms}`,
      "# HELP lingua_live_rooms Current live rooms",
      "# TYPE lingua_live_rooms gauge",
      `lingua_live_rooms ${liveRooms}`,
      "# HELP lingua_listeners Current audience websocket listeners",
      "# TYPE lingua_listeners gauge",
      `lingua_listeners ${listeners}`,
      "# TYPE lingua_rooms_created_total counter",
      `lingua_rooms_created_total ${metrics.roomsCreated}`,
      "# TYPE lingua_rooms_started_total counter",
      `lingua_rooms_started_total ${metrics.roomsStarted}`,
      "# TYPE lingua_rooms_ended_total counter",
      `lingua_rooms_ended_total ${metrics.roomsEnded}`,
      "# TYPE lingua_websocket_connections_total counter",
      `lingua_websocket_connections_total ${metrics.websocketConnections}`,
      "# TYPE lingua_transcript_events_total counter",
      `lingua_transcript_events_total ${metrics.transcriptEvents}`,
      "# TYPE lingua_audio_events_total counter",
      `lingua_audio_events_total ${metrics.audioEvents}`,
      "# TYPE lingua_upstream_errors_total counter",
      `lingua_upstream_errors_total ${metrics.upstreamErrors}`,
      "# TYPE lingua_backpressure_drops_total counter",
      `lingua_backpressure_drops_total ${metrics.listenerBackpressureDrops}`,
  ...
  app.post("/api/rooms", async (request, response, next) => {
    try {
      ...
      response.status(201).json({
        room: rooms.hostView(room),
        hostToken,
        inviteUrl: `${baseUrl}/audience/${room.code}`,
        embedUrl: `${baseUrl}/embed.html?room=${room.code}`,
        captionsUrl: `${baseUrl}/captions.html?room=${room.code}`,
      });
    } catch (error) {
      next(error);
    }
  ...
  app.get("/api/rooms/:code", (request, response, next) => {
    try {
      response.json({ room: rooms.publicView(rooms.get(request.params.code)) });
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/rooms/:code/captions.sse", (request, response, next) => {
    try {
      const room = rooms.get(request.params.code);
      rooms.verifyAudienceAdmission(room, request.query.admission);
      ...
      sseSend(response, "ready", { room: rooms.publicView(room) });
  ...
  app.get("/api/rooms/:code/captions.vtt", (request, response, next) => {
    try {
      ...
      const cues = completed.map((event, index) => {
        const start = Math.max(0, (event.at || baseTime + index * 3000) - baseTime);
  ...
  app.get("/api/rooms/:code/qr", async (request, response, next) => {
    try {
      ...
      const configuredBase = options.publicBaseUrl ?? process.env.PUBLIC_BASE_URL;
      const baseUrl = (configuredBase || `${request.protocol}://${request.get("host")}`).replace(/\/$/, "");
      const image = await QRCode.toBuffer(`${baseUrl}/audience/${room.code}`, {
        type: "png",
        width: 360,
        margin: 2,
        color: { dark: "#14213d", light: "#ffffffff" },
      });
  ...
  app.get("/api/rooms/:code/transcript.csv", (request, response, next) => {
    try {
      ...
      const csv = [
        "timestamp,source,translation",
        ...rows.map((row) => [csvEscape(row.timestamp), csvEscape(row.source), csvEscape(row.translation)].join(",")),
      ].join("\n");
      response
        .type("text/csv")
        .set("Content-Disposition", `attachment; filename=lingua-live-${room.code}.csv`)
        .send(csv);
    } catch (error) {
      next(error);
    }
  ...
  function emitSseCaption(room, event) {
    for (const response of room.sseClients || []) {
      try {
        sseSend(response, "caption", event);
      } catch {
        room.sseClients.delete(response);
      }
    }
  ...
  async function startRoom(room) {
    ...
    operation
      .finally(() => {
        if (room.startPromise === operation) room.startPromise = null;
      })
      .catch(() => {});
    return operation;
  }
  ...
  app.use((error, _request, response, _next) => {
    if (error instanceof SyntaxError && error.status === 400) {
      error = new AppError(400, "INVALID_JSON", "The request body is not valid JSON.");
    }
    const status = error instanceof AppError ? error.status : 500;
  ...
  server.on("upgrade", (request, socket, head) => {
    try {
      ...
      if (!new Set(["host", "audience"]).has(role)) {
        throw new AppError(400, "INVALID_ROLE", "Role must be host or audience.");
      }
      ...
      webSockets.handleUpgrade(request, socket, head, (ws) => {
        webSockets.emit("connection", ws, request, { room, role, clientId });
      });
    } catch (error) {
      upgradeError(socket, error);
    }
  });
  ...
  webSockets.on("connection", (ws, _request, { room, role, clientId }) => {
    ...
    sendJson(ws, {
      type: "session.ready",
      role,
      room: role === "host" ? rooms.hostView(room) : rooms.publicView(room),
      history: role === "audience" ? room.history : undefined,
    ...
    ws.on("message", (data) => {
      try {
        ...
        if (role !== "host") {
          throw new AppError(403, "FORBIDDEN_MESSAGE", "Audience connections cannot send host controls or audio.");
        }
        if (message.type !== "audio.append") {
          throw new AppError(400, "UNKNOWN_MESSAGE", "Unknown host message type.");
        }
        if (room.status !== "live" || !room.realtime) {
          throw new AppError(409, "ROOM_NOT_LIVE", "Start interpretation before sending audio.", { retriable: true });
        }
        room.realtime.appendAudio(message.audio);
      } catch (error) {
        if (error instanceof SyntaxError) {
          error = new AppError(400, "INVALID_MESSAGE_JSON", "WebSocket messages must be valid JSON.");
        }
